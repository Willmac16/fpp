...
_]_
_[_
`bitfield` `{` _bitfield-field-sequence_ `}`
_]_

_bitfield-field-sequence_ is an <<Element-Sequences,element sequence>>
in which the elements are bitfield fields, and the terminating
punctuation is a comma.
A *bitfield field* has the following syntax:

<<Lexical-Elements_Identifiers,_identifier_>> `:`
<<Expressions_Integer-Literals,_integer-literal_>>
...
The optional bitfield specifier for a struct member specifies a list
of named bitfield fields within the member.
If present, the member type must be a primitive integer type
(`I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64`).
Each bitfield field size must be greater than zero,
and the total size of all the fields must be less than or equal to
the bit width of the member type.
Fields are ordered from least significant to most significant bit
...
# Defines a struct type E with bitfield member flags
struct E {
