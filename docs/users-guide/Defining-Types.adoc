...
For any struct member with a primitive integer type, you can include an
optional bitfield specification. To do this, write the keyword `bitfield`
followed by a list of field names and sizes enclosed in braces `{` ... `}`.
Each field size is an integer literal giving the number of bits.
...
----
struct Flags {
  status: U16 bitfield { enabled: 1, mode: 3, prio: 4, reserved: 8 }
}
----

Each field size must be greater than zero, and the sum of the field sizes
must be less than or equal to the bit width of the member type.
