...
[source,bash]
----
% export FPP_JAVA_HOME=/Library/Java/JavaVirtualMachines/openjdk21/Contents/Home
----
...
For example, download Oracle GraalVM for JDK 21 from
https://www.graalvm.org/downloads/ and unpack it.
+
NOTE: Oracle GraalVM ships `-O3` and profile-guided optimization (PGO).
GraalVM Community Edition does not. A plain `./release` works with either;
the `perf/*` experiment branches require Oracle GraalVM.

. Set the environment variables `GRAALVM_JAVA_HOME` (read by `release`) and
`FPP_GRAALVM_JAVA_HOME` (read by `install-trace`) to the `Home` directory
associated with the GraalVM installation in step 1.
For example, on Mac OS:
+
[source,bash]
----
% export GRAALVM_JAVA_HOME=/Library/Java/JavaVirtualMachines/graalvm-jdk-21/Contents/Home
% export FPP_GRAALVM_JAVA_HOME="$GRAALVM_JAVA_HOME"
----

NOTE: As of GraalVM for JDK 17 and later, `native-image` ships inside the
distribution. The old `gu install native-image` step no longer applies, and
`gu` is not present.
...
. Run `./trace-fprime` to extend the trace with real F Prime models:
+
[source,bash]
----
% export FPRIME=[path-to-fprime-checkout]
% ./trace-fprime
----
+
IMPORTANT: Do not skip this step. `./test` only exercises the compiler's own
fixtures. `./trace-fprime` covers the code paths that only real framework and
topology models reach — in particular JSON model generation
(`fpp-to-json`, backed by circe), which is the most reflection-dependent part
of the compiler. Regenerating `reflect-config.json` from `./test` alone
produces a smaller config than the one already committed, and a native image
that builds clean and then fails at runtime on real models.

. Compare the regenerated `reflect-config.json` against the previous one before
committing. The entry count should not go *down*.
