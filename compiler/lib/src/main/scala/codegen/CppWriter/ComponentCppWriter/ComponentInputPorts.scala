...
case class ComponentInputPorts(
  ...
  // Writes the lines that empty the caller's handle on any buffer whose ownership this call transfers.
  //
  ...
  private def writeTransferredParamRelinquishLines(p: PortInstance): List[Line] = {
    ...
    guardedList (transferred.nonEmpty) (
      transferred.flatMap(
        n => lines(
          s"""|// The message is on the queue, so $n now belongs to the far side of it. Emptying the caller's handle
              |// here is what keeps a buffer from being referred to twice; the queue carries the ownership state
